...
/**
 * Busca os animes cujo nome bate (substring ou aproximado) com a query,
 * varrendo TODAS as pastas de letra dentro de "Animes/" em paralelo.
 *
 * Importante: não dá pra usar a primeira letra da query pra escolher só
 * uma pasta de letra, porque a pasta é organizada pela primeira letra do
 * TÍTULO do anime, não da busca — "Chou kaguya-hime" fica em "C", então
 * uma busca por "kaguya" (substring, não prefixo) só encontra isso
 * varrendo todas as pastas.
 */
async function findAnimeMatches(query) {
  ...
  // Lista o conteúdo de todas as pastas de letra em paralelo (evita fazer
  // isso sequencialmente, já que agora sempre varremos tudo).
  const folderContents = await Promise.all(
    letterFolders.map(async (folder) => {
      const folderPath = `Animes/${folder.name}`;
      try {
        const data = await listFiles(folderPath);
        return { folderPath, items: (data.files || []).filter((f) => f.is_directory !== false) };
      } catch (err) {
        console.log(`[debug] falha ao listar "${folderPath}": ${err.message}`);
  ...
  const matches = [];
  for (const { folderPath, items } of folderContents) {
